...
def removing_non_korean(df):
    for idx, row in tqdm(df.iterrows(), desc='removing_non_korean', total=len(df)):
        new_doc = re.sub('[^가-힣]', '', row['text']).strip()
        df.loc[idx, 'text'] = new_doc
    return df
...
def generate_preprocessed():
    ...
    s3 = boto3.client('s3')
    s3.download_file('wilcrawling', f'dataset_{today}.txt', f'../data/dataset_{today}.txt')
...
#     data = removing_non_korean(data)
    ...
    # 파일 저장
    train.to_csv(f'{data_path}/train_{today}.txt', index=False, sep='\t')
    test.to_csv(f'{data_path}/test_{today}.txt', index=False, sep='\t')
